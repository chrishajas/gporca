	...
	)
{
	...
	return 0 < pexpr->GetDrvdPropRelational().PcrsOuter()->Size();
}
	...
	)
{
	...
	CColRefSet *outer_refs = exprhdl.GetRelationalProperties(1 /*child_index*/)->PcrsOuter(exprhdl);
	...
	)
{
	return FInnerUsesExternalCols(exprhdl) &&
			exprhdl.GetRelationalProperties(1)->PcrsOuter(exprhdl)->IsDisjoint(exprhdl.GetRelationalProperties(0)->PcrsOutput());
}
