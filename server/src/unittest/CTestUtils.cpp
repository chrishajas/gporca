	...
	)
{
	...
	// get any two columns
	CColRefSet *pcrs = pexpr->PcrsOutput();
	...
	)
{
	...
	// get any two columns
	CColRefSet *pcrs = pexprGet->PcrsOutput();
	CColRef *pcrLeft =  pcrs->PcrAny();
	CColRef *pcrRight = pcrs->PcrAny();
	CExpression *pexprPredicate = CUtils::PexprScalarEqCmp(mp, pcrLeft, pcrRight);
	...
	)
{
	...
	CColRefSet *pcrsLeft = pexprLeft->PcrsOutput();
	CColRef *pcrLeft =  pcrsLeft->PcrAny();

	CColRefSet *pcrsRight = pexprRight->PcrsOutput();
	...
	)
{
	...
	CColRef *pcrLeft1 = pexprLOJInnerChild->PcrsOutput()->PcrAny();

	// get a random column from NAry join second child
	CColRef *pcrLeft2 = (*pexprNAryJoin)[1]->PcrsOutput()->PcrAny();

	// get a random column from NAry join output
	CColRef *pcrRight = pexprNAryJoin->PcrsOutput()->PcrAny();
...
{
	CExpression *pexprNAryJoin = PexprLogicalNAryJoin(mp);
	CColRef *pcrNAryJoin = (*pexprNAryJoin)[0]->PcrsOutput()->PcrAny();
	...
	CColRef *pcrLeft = pexprLOJOuterChild->PcrsOutput()->PcrAny();

	// get a random column from LOJ inner child output
	CColRef *pcrRight = pexprLOJInnerChild->PcrsOutput()->PcrAny();
	...
	)
{
	...
	CColRefSet *pcrs = pexprInput->PcrsOutput();
	...
	)
{
	...
	CColRefArray *colref_array = GPOS_NEW(mp) CColRefArray(mp);
	CColRef *colref = pexprJoin->PcrsOutput()->PcrAny();
	...
	)
{
	...
	CColRefSet *pcrs = pexprOuter->PcrsOutput();
	...
	)
{
	...
	// include one grouping column
	CColRefArray *colref_array = GPOS_NEW(mp) CColRefArray(mp);
	CColRef *colref = pexpr->PcrsOutput()->PcrAny();
	colref_array->Append(colref);
	...
	)
{
	...
	CColRefSet *pcrs = pexprCTG->PcrsOutput();
	...
	)
{
	...
	CColRefArray *colref_array = pexprSelect->PcrsOutput()->Pdrgpcr(mp);
	...
	)
{
	...
	CExpressionArray *pdrgpexpr = GPOS_NEW(mp) CExpressionArray(mp);
	pdrgpexpr->Append(CUtils::PexprScalarIdent(mp, pexprInput->PcrsOutput()->PcrAny()));
